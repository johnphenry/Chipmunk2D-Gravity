...
func createBodies() {
	...
	staticLines = []*chipmunk.Shape{
		//chipmunk.NewSegment(vect.Vect{111.0, 280.0}, vect.Vect{407.0, 280.0}, 0),
		chipmunk.NewSegment(vect.Vect{0, 0}, vect.Vect{400.0, 400.0}, 0),
...
func main() {
	...
	for !window.ShouldClose() {
		state:= window.GetKey( glfw.KeySpace)
		if state == glfw.Press{
			space.Gravity = vect.Vect{0, 0}
		}else{
			space.Gravity = vect.Vect{0, -900}
		}
		state = window.GetKey( glfw.KeyE)
		if state == glfw.Press{
			addBall()
